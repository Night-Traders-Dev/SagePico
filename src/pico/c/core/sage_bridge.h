...
static SageValue sage_ffi_srvm_run_wrap(int argc, SageValue* argv) {
    ...
    if (!gfx_active_vm) {
        static gfx_vm_t srvm_vm; extern uint8_t* disp_fb;
        if (!disp_fb) { static uint8_t dummy_fb[640*400]; disp_fb = dummy_fb; }
        gfx_vm_init(&srvm_vm, disp_fb); gfx_active_vm = &srvm_vm;
    }
    ...
    rvvm_run(vm, 100000);
    return sage_number((double)(vm->cycles - start_cycles));
}
/* RTOS wrappers */
static SageValue sage_ffi_rtos_task_wrap(int argc, SageValue* argv) {
    (void)argc; /* stub — fn pointer not passable via FFI */
    return sage_number((double)sagertos_task_create(NULL, 128, 1));
}
static SageValue sage_ffi_rtos_sleep_wrap(int argc, SageValue* argv) {
    sagertos_sleep_ms((uint32_t)sv_int(argv[0])); return sage_nil();
}
static SageValue sage_ffi_rtos_yield_wrap(int argc, SageValue* argv) {
    (void)argc; (void)argv; sagertos_yield(); return sage_nil();
}
static SageValue sage_ffi_rtos_id_wrap(int argc, SageValue* argv) {
    (void)argc; (void)argv; return sage_number((double)sagertos_task_id());
}
/* Watchdog wrappers */
...
static const SageFFIEntry sage_ffi_table[] = {
    ...
    FFI_ENTRY(FFI_HANDLE_PICO, "rtos_task",    sage_ffi_rtos_task_wrap),
    FFI_ENTRY(FFI_HANDLE_PICO, "rtos_sleep",   sage_ffi_rtos_sleep_wrap),
    FFI_ENTRY(FFI_HANDLE_PICO, "rtos_yield",   sage_ffi_rtos_yield_wrap),
    FFI_ENTRY(FFI_HANDLE_PICO, "rtos_id",      sage_ffi_rtos_id_wrap),
};
